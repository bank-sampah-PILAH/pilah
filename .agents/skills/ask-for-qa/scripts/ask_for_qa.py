#!/usr/bin/env python3
"""Assign a known GitHub QA reviewer and notify a Discord webhook."""
...
WEBHOOK_ENV = "DISCORD_QA_WEBHOOK_URL"
...
def normalize_bullets(values: list[str], label: str) -> list[str]:
    bullets = []
    for value in values:
        for line in value.splitlines():
            ...
            if item:
                bullets.append(item)
    if not bullets:
        raise UserError(f"at least one {label} bullet is required")
    return bullets
...
def message_content(
    ...
    changes = normalize_bullets(changes, "change")
    steps = normalize_bullets(steps, "verification step")
    lines = [
        f"[{area}] {title}",
        f"PR: {pr_url}",
        f"From: <@{sender.discord_id}>",
        f"To: <@{qa_member.discord_id}>",
...
def send_discord(webhook: str, content: str, sender: Member, qa_member: Member) -> None:
    payload = {
        "content": content,
        "allowed_mentions": {"users": [sender.discord_id, qa_member.discord_id]},
    }
    request = Request(
        webhook,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Assign a known GitHub QA reviewer and notify Discord."
    )
    parser.add_argument("--qa", required=True, help="GitHub login or known Discord alias")
    parser.add_argument(
        "--change",
        dest="changes",
        action="append",
        required=True,
        help="One change bullet; repeat for multiple bullets",
    )
    parser.add_argument(
        "--step",
        dest="steps",
        action="append",
        required=True,
        help="One verification-step bullet; repeat for multiple steps",
    )
...
def main() -> int:
    ...
    qa_member = resolve_member(args.qa)
    webhook = webhook_url()
    ...
    print(f"QA reviewer: {qa_member.github}")
    if args.dry_run:
    ...
    edit_args.extend(["--add-reviewer", qa_member.github])
    run_gh(edit_args)
    print(f"Assigned GitHub QA reviewer {qa_member.github}.")

    try:
        send_discord(webhook, content, sender, qa_member)
    except UserError as exc:
        raise UserError(
            f"QA assignment succeeded, but Discord notification failed: {exc}"
        ) from exc
    print("Discord QA request sent.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"ask-for-qa: {exc}", file=sys.stderr)
        raise SystemExit(1)
